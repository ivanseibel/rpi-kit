...
const ROOT_PREFIXES = [".agents/", ".rpi/"];

const EXCLUDED_REPO_REL_PREFIXES = [".rpi/projects/", ".rpi/scripts/"];
const EXCLUDED_REPO_REL_EXACT = new Set([
	".rpi/projects",
	".rpi/scripts",
]);
...
function isExcludedRepoRelPath(repoRelPath) {
	const normalized = repoRelPath
		.trim()
		.replace(/^\.\//, "")
		.replace(/^\/+/g, "");

	if (EXCLUDED_REPO_REL_EXACT.has(normalized)) {
		return true;
	}

	for (const prefix of EXCLUDED_REPO_REL_PREFIXES) {
...
function toPosixRelativePath(relPath) {
...
async function main() {
	...
	if (!fs.existsSync(path.join(kitRoot, ".github"))) {
		// Historically this kit was structured around .github; Codex-centric layout does not require it.
		// Keep a minimal sanity check that we're running from the kit root.
		if (!fs.existsSync(path.join(kitRoot, "AGENTS.md"))) {
			throw new Error("Kit is missing AGENTS.md; run from kit root.");
		}
	}
	...
	const scanDirs = [
		path.join(kitRoot, ".agents/skills"),
		path.join(kitRoot, ".rpi/docs"),
	];
	...
	if (fs.existsSync(templateDir) && isDirectory(fs.statSync(templateDir))) {
		...
		for (const template of templateFiles) {
			...
			if (isExcludedRepoRelPath(targetPath)) {
				continue;
			}
	...
	for (const src of files) {
		const relPosix = toPosixRelativePath(path.relative(kitRoot, src));
		if (isExcludedRepoRelPath(relPosix)) {
			continue;
		}
		if (relPosix.startsWith(".github/workflows/")) {
			continue;
		}
		if (templateTargets.has(relPosix)) {
			continue;
		}

		const override = fileOverrides.get(relPosix);

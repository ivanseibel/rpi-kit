Based on the RPI operational doctrine, here is the technical definition file for the Plan Phase.
...
These constraints are enforced via agent configuration and process governance.  
Constraint,Definition,Technical Enforcement  
No Production Code,The Planner must not write application code. It only writes instructions for code.,"Use a read-only tool set in the Plan phase (no file edits outside plan.md)."  
...
`.agents/skills/rpi-workflow/resources/plan-template.md`.

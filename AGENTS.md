...
4. **Only approved install paths.** The only canonical installation paths are `~/.copilot/skills/` (copilot), `~/.agents/skills/` (codex), and `~/.gemini/antigravity/skills/` (antigravity). Do not introduce other tool-specific paths such as `~/.claude/`, `.github/skills/`, or `.agents/skills/` for copilot.

Based on the RPI operational doctrine, here is the technical definition file for the Research Phase.
...
To ensure RPI integrity, the following constraints must be enforced via agent configuration and/or process governance:  
Constraint,Definition,Technical Enforcement  
Read-Only Access,"The agent/developer may read files, grep logs, and search docs, but MUST NOT modify a single line of code.","Use a read-only tool set in the Research phase (no file edits)."  
No Solutioning,"The output must describe what exists, not what will be built. No architecture diagrams or pseudocode.","Governance: Research output must be facts/patterns only."  
Citations Required,"Every claim regarding system behavior must be backed by a file path, line number, or documentation link.","Process requirement: all assertions must be cited."
...
`.agents/skills/rpi-workflow/resources/research-template.md`.

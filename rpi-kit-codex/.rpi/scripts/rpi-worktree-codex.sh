...
usage() {
  cat <<'EOF'
Usage:
  rpi-worktree-codex.sh --project <path> --worktree <path> [options]
...
Options:
  ...
  --kit <path>               Path to rpi-kit-codex root (default: auto-detect)
...
echo "Installing Codex scaffold into worktree: $WORKTREE"
...
append_exclude_rule "$EXCLUDE_FILE" "/.agents/"
append_exclude_rule "$EXCLUDE_FILE" "/.rpi/AGENTS.md"
append_exclude_rule "$EXCLUDE_FILE" "/.rpi/docs/"
append_exclude_rule "$EXCLUDE_FILE" "/.rpi/reusable-prompts/"
...
if [ -n "$blocked" ]; then
  echo "Commit blocked: Codex scaffold files are local-only in this worktree." >&2
...
Verification commands:
  ...
  git -C "$WORKTREE" add -f .agents/skills/rpi-workflow/SKILL.md
